...
data social3; set social2; if missing(var1)=1; Rank_Anomaly=2; run; 
...
proc sort data=social4; by date var1; run;
...
proc sql;
  ...
  select distinct Date, AnomalyVar, Rank_Anomaly, mean(Ret1) as EWHoldRet, sum(MarketCapitalization*Ret1)/sum(MarketCapitalization) as VWHoldRet
  from social4
...
proc sql;
	create table social4 as
	select distinct *, std(var1) as std, var1/std(var1) as sum_var2
	from social4 
	group by date;
quit;
...
data social4;
	set social4;
	if missing(sum_var2)=1 or missing(ret1)=1 then delete;
run;

proc sort data=social4; by date rank_anomaly;run;
...
/*proc sql;
	drop table social2, social3, social4, social5, social6, meanret, tstatret, social_port2, mean, tstat, alpha_fmb, social_port, teststats, teststats1;
quit;*/ 
